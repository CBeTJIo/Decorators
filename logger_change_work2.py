...
def logger(path):
    def __logger(old_function):
        ...
        def new_function(*args, **kwargs):
            ...
            today_time = datetime.time(datetime.today())
            date_function = today_date.strftime('%d.%m.%Y')
            time_function = today_time.strftime('%H:%M:%S')
            ...
            with open(f'{file_name}', 'a') as f:
                f.write(f'Вызов функции произошел {date_function} в {time_function}.'
                        f'Имя функции {name_function} с аргументами {args} и {kwargs}.'
